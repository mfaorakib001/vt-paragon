 <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
              ...
              <div class="ps-3">
                <nav aria-label="breadcrumb">
                  <ol class="breadcrumb mb-0 p-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">All Partner Report</li>
                  </ol>
                </nav>
              </div>
			...
			{
				$sql1 = "SELECT COUNT(*) a FROM `member`  where `member_ref`!=''";
			...
			if($role=="Administrator")
			{
				$sql1 = "SELECT COUNT(*) a FROM `member`  where `member_ref`!='' and month(`member_dt`)=month(now())-1";
				$result1 = $conn->query($sql1);
				if ($result1->num_rows > 0) 
				{
				  while($row1 = $result1->fetch_assoc()) 
			...
			if($role=="Administrator")
			{
				$sql1 = "SELECT COUNT(*) a FROM `member`  where `member_ref`!='' and month(`member_dt`)=month(now())";
				$result1 = $conn->query($sql1);
				if ($result1->num_rows > 0) 
				{
				  while($row1 = $result1->fetch_assoc()) 
